<template>
    <header class="sticky bg-charcoal top-0 z-50 shadow-xl">
        <div class="flex p-3 h-24 z-50 items-center justify-between">
            <AppLogo/>
            <span v-if="user" class="text-white">Bienvenue, {{user.first_name}}</span>
        ...
        <div v-show="menu" class="absolute right-0 h-[calc(100vh-6rem)] bg-platinum flex flex-col justify-between items-center w-full overflow-scroll">
            <nav>
                <ul class="space-y-12 pt-12">
                    ...
                    <li v-if="user && ! user.is_admin">
                        <NavLink @click="menu = !menu" :active="false" :href="route('account')">Mon compte</NavLink>
                    </li>
                    <li v-if="!user">
